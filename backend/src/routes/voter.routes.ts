...
function parseSelfieBase64(selfieB64: string): Buffer {
...
function sameRegion(a: string, b: string, c: string, d: string): boolean {
  return (
...
// FaceVerifyService now converts all connectivity/AI errors to
// AiServiceUnavailableError before they reach this route, so plain
// instanceof checks are sufficient — no AggregateError handling needed here.
...
router.get("/elections", requireVoter, async (req: any, res, next) => {
...
router.get("/requests", requireVoter, async (req: any, res, next) => {
...
router.get("/notifications", requireVoter, async (req: any, res, next) => {
  try {
    const page = Math.max(1, parseInt(req.query.page as string) || 1);
    const limit = Math.max(
      1,
  ...
  async (req: any, res, next) => {
    ...
    try {
      await db.withTransaction(async (client) => {
        ...
        // Get withdrawal deadline hours separately to avoid LEFT JOIN in FOR UPDATE
          ...
          if (selfie_b64 && selfieBuf) {
            ...
            if (blinkFramesRaw) {
              try {
                ...
                // checkBlink fails-open internally, so this catch only fires for
                // JSON.parse errors on malformed blink_frames. Re-throw real
                // infrastructure errors so they surface correctly.
