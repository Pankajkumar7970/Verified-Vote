...
router.get('/candidates', requireValidSession('face_verified'), async (req: any, res, next) => {
...
router.post('/cast', requireValidSession('face_verified'), validate(castVoteSchema), async (req: any, res, next) => {
  const session = req.votingSession;
