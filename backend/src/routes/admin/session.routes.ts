...
router.get('/', requireAdmin, requireSuperAdmin, async (req: any, res, next) => {
  try {
    ...
    // Try to fetch with voting_selfie_minio_key, if that fails, fetch without it
    let result;
    try {
    ...
    const sessions = await Promise.all(
      result.rows.map(async (session) => {
        const { request_selfie_minio_key, voting_selfie_minio_key, ...rest } = session;
...
router.post('/:id/revoke', requireAdmin, requireSuperAdmin, validate(revokeSessionSchema), async (req: any, res, next) => {
...
router.post('/:id/approve-face', requireAdmin, validate(approveFaceSchema), async (req: any, res, next) => {
  const { note } = req.body;
  const safeNote = sanitizeHtml(note, { allowedTags: [], allowedAttributes: {} });
