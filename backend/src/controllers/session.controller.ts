/**
 * Voting session handlers: ref-code resolution, OTP, and face verification.
 */
...
export async function getStatus(req: any, res: Response, next: NextFunction) {
  ...
  try {
    ...
    const canResume = ["otp_verified", "face_verified", "face_pending"].includes(
      session.state,
    );
...
export async function faceVerify(req: any, res: Response, next: NextFunction) {
