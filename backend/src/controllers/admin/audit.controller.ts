/**
 * Admin audit log handlers: paginated list and CSV export.
 */
...
export async function exportAuditLogs(
  ...
  try {
    ...
    const header =
      "timestamp,action,actor_type,actor_id,entity_type,entity_id,ip,metadata,request_id\n";
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="audit-export-${checksum.slice(0, 8)}.csv"`,
    );
...
export async function listAuditLogs(
  ...
  try {
    ...
    const limit = Math.max(
      1,
      Math.min(500, parseInt(req.query.limit as string) || 20),
    );
    const offset = (page - 1) * limit;

    const { actor_type, action, entity_type, ip_address, start_date, end_date, search } =
      req.query;
